-- The ingestr benchmark table for a MySQL source — same 15 columns and the same value
-- generators as seed.sql (Postgres), ported to MySQL 8 dialect so cross-source runs
-- move identical data. Placeholders: BENCH_TABLE_PLACEHOLDER, BENCH_ROWS_PLACEHOLDER.
-- float_val differs in the last bits (MySQL's `/` is decimal division, Postgres' is
-- float) — float sums are excluded from cross-engine checksums anyway.
