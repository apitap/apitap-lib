...
// apitap patch: upstream uses 8192 (the tokio/std default), which for a bulk-transfer
// workload means ~150k recv(2) syscalls per GB moved and constant buffer reshuffling.
// 512 KiB cuts that ~64x for ~1 MB of heap per connection (apitap holds ≤ 33
// connections at its widest tier). Applies to both buffers and both shrink floors.
const DEFAULT_BUF_SIZE: usize = 512 * 1024;
